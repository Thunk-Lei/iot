...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueAltitude != cur->valueAltitude)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueAltitude Changed\n");
            ret = 1;
        }
    }
    ...
    if(last->valueRainfall != cur->valueRainfall)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueRainfall Changed\n");
            ret = 1;
        }
    }
    if(last->valuePressure != cur->valuePressure)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuePressure Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueAltitude = exchangeBytes(gizY2X(Altitude_RATIO,  Altitude_ADDITION, dataPoints->valueAltitude)); 
    devStatusPtr->valueWindspeed = exchangeBytes(gizY2XFloat(Windspeed_RATIO,  Windspeed_ADDITION, dataPoints->valueWindspeed));     
    devStatusPtr->valueRainfall = exchangeBytes(gizY2X(Rainfall_RATIO,  Rainfall_ADDITION, dataPoints->valueRainfall)); 

    devStatusPtr->valuePressure = exchangeWord(gizY2XFloat(Pressure_RATIO,  Pressure_ADDITION, dataPoints->valuePressure)); 
    devStatusPtr->valueLux = exchangeWord(gizY2X(Lux_RATIO,  Lux_ADDITION, dataPoints->valueLux)); 
